...
func (g *scrollGestureGuard) observe(list *layout.List) {
	g.observeState(list.Position, list.Dragging())
}

func (g *scrollGestureGuard) observeState(position layout.Position, dragging bool) {
	moved := position.First != g.position.First || position.Offset != g.position.Offset
	// A layout refresh or programmatic ScrollTo can move a list without any
	// touch momentum. Only continue inertia that was already observed or arm it
	// after an actual moving drag; otherwise the next tap gets discarded.
	if dragging {
		g.dragMoved = g.dragMoved || moved
		g.inertial = false
		g.inertiaCandidate = false
	} else {
		candidate := g.inertial || g.inertiaCandidate || (g.wasDragging && g.dragMoved)
		g.inertial = moved && candidate
		g.inertiaCandidate = g.wasDragging && g.dragMoved && !moved
		g.dragMoved = false
	}
